/**
 * Canonical surface census — the SINGLE SOURCE OF TRUTH for every route/state
 * covered by the rendered test and review portfolio.
 *
 ...
 * census, so the two can never drift: every visual/polish surface is declared
 ...
 * add a `SurfaceCensusEntry` here or in a registered slice-owned fragment (and
 * its `ready`/`prepare` compatibility runtime in surfaces.ts during migration).
...
/**
 * Frozen legacy routes, grouped by kind for readability. Original order is
 * load-bearing during migration. Each route MUST be reachable from router.tsx.
 */
const LEGACY_SURFACE_ROUTES: readonly SurfaceRoute[] = [
  // ─── Pre-auth ───────────────────────────────────────────────────────────────
...
/**
 * Classify the frozen pre-Tactical inventory by its owning atlas board. New UI
 * slices add typed census fragments instead of extending this legacy adapter.
 */
function legacyBoardFor(id: string): AtlasBoard {
  if (
    id === "login" ||
    id.startsWith("shared-character") ||
    id === "not-found" ||
    id.startsWith("error-") ||
    id.startsWith("legal-page")
  ) {
    return "A01";
  }
  if (id === "home" || id.startsWith("roster-")) return "A02";
  if (id === "create" || id.startsWith("create-guided")) return "A04";
  if (id.startsWith("level-up")) return "A09";
  if (id === "campaigns" || id === "campaign-create" || id === "campaign-join") {
    return "A10";
  }
  if (
    id.startsWith("campaign-encounter") ||
    id === "campaign-monster-statblock" ||
    id === "campaign-hub-encounter"
  ) {
    return "A13";
  }
  if (
    id === "campaign-hub-chronicle-edit" ||
    id === "campaign-hub-session-edit" ||
    id === "campaign-hub-note-edit" ||
    id === "campaign-hub-treasury-add"
  ) {
    return "A12";
  }
  if (id.startsWith("campaign-")) return "A11";
  if (id.startsWith("compendium")) return "A14";
  if (id.startsWith("admin-") || id === "settings-page" || id === "account-menu") {
    return "A15";
  }
  if (id === "command-palette" || id === "report-dialog" || id === "shortcuts-sheet") {
    return "A16";
  }
  if (
    id.includes("spell") ||
    ...
    id.includes("feature") ||
    id.includes("mastery") ||
    id.includes("maneuver") ||
    id.includes("cast") ||
    id.includes("recovery") ||
    id.includes("pool-spend") ||
    id.includes("payment-picker")
  ) {
    return "A07";
  }
  if (
...
/** Canonical, Playwright-free census consumed by future slice-owned fragments. */
export const SURFACE_CENSUS: readonly SurfaceCensusEntry[] = LEGACY_SURFACE_ROUTES.map(
  ({ slug, route }) => ({
    id: slug,
    board: legacyBoardFor(slug),
    route,
    state: slug,
    variants: PAIRWISE,
    authority: "DESIGN.md; Tactical Codex UI/UX Wayfinder",
    callSite: `tests/e2e/surfaces.ts#${slug}`,
    curatedReview: true,
  })
);

assertSurfaceCensus(SURFACE_CENSUS);

/** Compatibility adapter for existing route guards and Playwright traversals. */
export const SURFACE_ROUTES: readonly SurfaceRoute[] = SURFACE_CENSUS.map(
  ({ id: slug, route }) => ({ slug, route })
);
