...
function advanceFrame(
  ...
  return {
    ...
    trigger: { kind: "hit-points-zero", target: TARGET },
    ...overrides,
  };
}
...
describe("mechanics command suspension boundary", () => {
  ...
  it("binds create to the exact installation, root material, epoch, and invocation", () => {
    ...
    expect(
      conformMechanicsCommandSuspension(
        createSuspension({
          frame: createFrame({ trigger: { kind: "hit-points-zero", target: TARGET } }),
        })
      )
    ).toBeNull();
  });
  ...
  it("accepts advance from zero/null and a later exact one-step execution", () => {
    ...
    expect(
      conformMechanicsCommandSuspension(advanceSuspension({ rootReceipt: laterReceipt }))
    ).not.toBeNull();
  });
  ...
  it("binds advance to its root, authority, authored phase, and trigger role", () => {
    ...
    expect(
      conformMechanicsCommandSuspension(
        advanceSuspension({
          trigger: { kind: "hit-points-zero", target: OWNER },
        })
      )
    ).toBeNull();
    expect(
