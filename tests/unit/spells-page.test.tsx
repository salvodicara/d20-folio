...
function load(doc: CharacterDoc = structuredClone(MOCK_CHARACTER)): void {
  ...
  // Authenticated character, matching the live sheet context.
...
describe("SpellsTab", () => {
  ...
  it("allows successive casts without a combat turn gate", async () => {
    ...
    expect(useCombatStore.getState()).toMatchObject({
      selected: { action: [], bonus: [] },
      spellSlotCastsThisTurn: 0,
    });
    expect(useUndoStore.getState().past).toHaveLength(undoDepth + 1);
  });
