...
const ENTRY_CEILING_KB = 63; // baseline 53.7 → +17% (2026-07-10: +1 for the global keyboard-shortcut listener + the nav-anchor chrome; the SHORTCUTS registry itself stays in the lazy ShortcutsSheet chunk. 2026-07-24: raised 61 → 62 (+1 KB) for the ⌘K reference palette entries — the always-mounted palette's referenceHits memo + its inline bilingual search-term arrays + the requestPlayRef uiStore seam, all EAGER SHELL code. 2026-07-31: raised 62 → 63 (+1 KB) for the anonymous-viewer topbar chrome — the logged-out share-link CTA cluster ("Create your character" / "Get started" / "Sign in") the Topbar renders in place of the account cluster; all EAGER SHELL (the topbar is on the eager AppShell). NOT a data leak: the CTA links route to /login (no firebase/auth import), and the post-view conversion card lives in the LAZY SharedCharacterView chunk, not the entry; eager-closure chunk families unchanged vs main. Measured 62.07; +0.93 KB deterministic headroom (never exact-fit))
