...
**Encounter participation is explicit and forgiving** (owner 2026-08-03, superseding the former hard
initiative gate). During gathering, a player has one quiet **Skip encounter / Join encounter** action;
the DM may begin once at least one participant has rolled. A partial CTA says exactly **Begin with
{rolled}/{total}**, with the same crossed-swords grammar as the full start — no disabled lock theatre.
Unrolled PCs are opted out of the frozen order but remain on the encounter surface for targeting and DM
correction. The global chip disappears for an opted-out player, so the app never polls them for a roll
...
encounters idempotently, so legacy fights become first-class creatures without a migration prompt. Each
monster edits HP through the **same `.vital-hp` chip + shared `HpEditPopover`** the PC card uses — no
parallel widget (golden rule 10). Monsters have no temp pool, so the popover opens with **`hideTemp`** — a boolean prop that
...
**NPC allegiance is a first-class, overridable label.** The shared Add-creature dialog uses the canonical
`Segmented` control to choose Enemy/Ally once for both Bestiary and Custom tabs. Allied NPC cards wear one
quiet green outline badge, use the ally edge treatment, expose exact HP to players, and keep a compact DM
**Make enemy/ally** correction action. They are healing targets by default and do not inflate enemy XP or
block victory; **Any creature** remains the universal targeting escape hatch.
...
size="md">` in the `.topbar-brand` slot, never re-rendered or resized per auth state. The bar's
  `.topbar` shell (height / background / blur / border) is likewise untouched.
...
top border. **The seat line IS the panel's own border** (owner, 2026-08-01: "è semplicemente il
bordo del riquadro e avrebbe sempre dovuto esserlo"): the page hosts are normal folio panels —
...
active tab CLEAR of the 36px edge-dissolve zone. The tome composes
...
- **`ModalShell`:** centered, `--bg-surface-2`, `--elev-modal`, `--radius-xl`, scrim behind, at
  ...
  - **Scrollable body is keyboard-reachable.** The shared `.modal-body`/`DialogBody` is a
    ...
    child.
  - **Body grammar is structural, never caller discipline.** Every `ModalShell` routes through
    exactly one shared body tier: `ModalBody` for compact forms/prose, `ModalScrollColumn` for one
    tall reading region, or `ModalStage` for the reviewed compound applications whose shared child
    already owns the sanctioned `ModalScroll` (search/facets + results, crop canvas + controls).
    `ModalFoot` owns the action row. No raw wrapper, private `overflow-y-auto`, or hand-spaced action
    strip may sit directly under a shell. `modal-scroll.guard.test.ts` derives every `ModalShell`
...
**Held-resource target context (2026-08-04).** Target review keeps overview first but exposes the facts
that change an inspiration decision: a creature already holding a Bardic die or Heroic Inspiration says so directly on its
...
The **legal colophon leaf** (`/legal`, `.colophon`) composes the exact
`.tome-leaf-surface.cmp-tome` material as the Compendium — including the same per-theme texture booster
and dark-vellum/light-parchment selection, never a legal-only grain recipe. It spans the **full masthead
footprint** — the SAME page-shell width the framed
...
identity (sharp facets, 0–4px radii): operational marks are **geometric** — faceted diamond nodes,
gems and tapered hairlines — while the earned binding register may use restrained botanical
engraving when it comes from the verified ricalco pipeline. Everything is vector (external masks,
SVG data-URIs or pure CSS), theme-tintable, crisp at 1x and 2x. The grammar's placement rules:
...
- **Ornament marks the START of a rubric, a rail-head, slot/charge pips — and, since the
  ...
  diamond leads a section head, caps a rail, or sits in a socket; straight runs, ordinary section
  divider centres, and scrollbar ends stay QUIET. A border is a plain hairline; a divider fades to nothing at its
...
- **Reliquary corners (the Gilded Reliquary grammar — the full-BG3 push, owner-ratified
  ...
  The owner-approved current+B+BG3 refinement (2026-08-04) preserves the silhouette and registration
  but strikes it with a restrained warm gold→bronze ramp (never selection silver), so the existing
  pennacchio gains material depth without becoming a new ornament. Mechanism: the binding-corner spandrel silhouette (CC0-derived,
  `public/assets/ornaments/corner-*.svg`) rendered as four CSS mask layers painted with the
  shared warm gold→bronze ramp, on an overlay pseudo at `inset: -1px` — the tile origin
  ...
  consumes it.
- **Hero-seat weave (owner-approved 2026-08-04).** Top-level framed mastheads and dialog head bands
  seat the already-ratified CC0 centre-loop ricalco on the lower frame rule: 72×25px for mastheads,
  56×20px for dialogs. This is B's open negative space translated through BG3's quiet line grammar,
  not a second separator. The cockpit identity band keeps only the shared binding corners because
  its dense live-stat deck already occupies the seat's visual field. Ordinary sections remain nodeless.
...
| Turn step (campaign hub)  | Changing the active combatant changes only the struck-gold current-turn frame and `aria-current`; it never auto-expands a DM disclosure or calls `scrollIntoView`. Detail remains an explicit user choice, so the card list cannot reflow or move the page beneath the fixed Prev/Next controls.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
test.ts` READS the live `.cp-dest-chip` declaration out of `folio.css`, resolves its `color` /
   ...
   - ✅ **The active encounter summary is ONE encounter dossier, not a dashboard chip row.** Its
     command rail borrows the combat cards' struck-gold frame and material depth: round is a compact
     typographic marker, only difficulty keeps semantic badge color, XP is terse metadata, and End
   ...
   - ✅ **Battle resolution is one compact decision surface.** The shared `ModalShell` resolver leads
     ...
     adjustments—without explanatory copy. A condition this target resists from the current exact source
     is labeled **Immune** in the existing condition chooser and is omitted from automatic defaults; it is
     not a separate banner, and manual selection remains the deliberate override. A recurring target-bound effect opens on its actual creature;
...
| `parchment.webp` + `vellum-dark.webp` remaster                                  | 1672×941 each       | **SHIPPED, corrected 2026-08-03.** `vellum-dark.webp` comes from PROMPT_29; the light leaf now comes from the owner's exact pale PROMPT_28 source (WebP q50 + sharp_yuv, 17.5 KiB). The theme indirection remains `--tome-leaf`: light resolves to parchment, dark to vellum.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `public/assets/textures/parchment.webp`                                         | 1672×941, 17.5 KiB  | Compendium "ancient tome" cover (`--asset-parchment`) — the LIGHT-theme leaf. Re-encoded directly from the owner-supplied `PROMPT_28.png` at WebP q50 + sharp_yuv on 2026-08-03; no creative regrading. **Shipped.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `public/assets/textures/vellum-dark.webp`                                       | 1024×1024, < 150 KB | Seam-blended dark vellum (`--asset-vellum-dark`), colour-graded into the **#241c12 → #3a2d21** candlelit-umber band (mean ~#2d2216). **DARK theme only:** IS the `.cmp-tome` surface via the `--tome-leaf` indirection (light keeps `parchment.webp`). Ships ~19 KB. **Shipped.** **Remastered 2026-08-01 (PROMPT_29): 1672px source, graded to the same band, webp q85 — PSNR 40 dB vs source, 82 KB — the anti-grain pass.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `public/assets/brand/crest.webp`                                                | 349×384, < 40 KB    | The engraved brand crest (owner-generated P6): a bronze d20 in a laurel + acanthus wreath, shipped as an **alpha mask** (white ink, alpha = engraving luminance) and painted through CSS `mask-image` with the theme's gold as ink (`--asset-crest` → `.page-head-crest`), so ONE file wears each theme's own metal. Seated as the folio's **frontispiece watermark**: every framed realm masthead that sits on the standard app field — roster, campaigns list, compendium, settings, admin, legal — carries it via the `PageHeader` `crest` prop. The owner-approved hybrid calibration (2026-08-04) keeps it deliberately subordinate to title and actions: dark uses the accent at **0.055**; light uses the warm **Ember Penumbra umber** at **0.14**. The light crest remains discoverable as scorched filigree but no longer becomes a second focal point. The art-backed campaign hub is the ONE exception: its backdrop is the campaign's own art, so that art is the frontispiece and it omits the crest. Ships ~30 KB. **Shipped.** |
...
3. **Parchment** (the reading register — compendium tome, book spreads): the `#e0d0b0/#d0c0a0`
   ...
   **TWO-STYLE LAW (owner, 2026-08-01 — "due stili distinti come BG3, niente compromessi"):**
   parchment belongs to the COMPENDIUM alone. The character sheet's center column (`.sheet-page`)
   wears its OWN working-register plate, **CSS-BUILT** (the `.sheet-page` material, folio.css):
   the BG3-examine tones measured from the corpus (head ≈ srgb(32,28,25) melting to a
   ≈ srgb(19,14,11) body; light sibling #ded2bc → #c9bda6) as PX-ANCHORED gradient stops.
   **BACKGROUND-STABILITY LAW (owner: the "texture that regenerates" jump):** a surface whose
   height GROWS with content (the sheet page, any expanding panel) must never wear a
   `background-size: cover` image — cover rescales on every growth. Growing surfaces take
   px-anchored CSS gradients (or fixed-size tiles); photographic plates are reserved for
   fixed-viewport surfaces (the tome, scene backdrops). The index-tab construction is
   material-agnostic by design: `--page-tone` recalibrates per surface, nothing else moves.
   **ANTI-GRAIN LAW (owner, 2026-08-01):** any asset painted with `cover` over a large fixed
   surface ships at ≥2× its largest CSS coverage, texture contrast σ per 100×100 patch < ~7 so
   text on top is never compromised. The compendium leaves are queued for 2× remasters
   (PROMPT_28/29) under this law.
...
- The hero-seat form spends that same traced weave once on the lower rule of a framed realm
  masthead (72×25px) or dialog head (56×20px). It never repeats on the cockpit identity band or
  ordinary section dividers; density earns silence.
