...
| 2     | The material primitives — PLATE (one specular dome) · EDGE (**moat → metal → groove → body**; there is no cream lip) · HAIRLINE · INK, replacing `--elev-*` on every plate. **9 tokens**, the closed set pinned by `chrome-system.guard.test.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | **DONE** |
...
| 6     | **The MARK** — the corner terminal redrawn as the **glint fan alone**, seated INSIDE the 10px radius (never crossing the vertex, never re-drawing the host's own rail), plus the run cartouche; mounted on exactly ONE surface per route — the framed masthead, or the cockpit's identity band where no masthead renders. Carries the shipped taste rulings forward: the metal is **dimensional, not line-art** (every struck member has a light/shade pair — dark = gold-300 body on a near-black under-shadow seat below-right with a gold-200 glint above-left; light = **GOLD, not bronze** (owner 2026-07-24), a #94741f antique-gold body inverted to letterpress logic: warm-cream understroke below-right, faint umber shadow wall above-left), and each corner mirrors the UNFILLED master geometry first and tones AFTER in screen space so the bevel light stays top-left on all four. Mechanism when it lands: fixed-size per-corner SVG background layers on an overlay pseudo — **never** `border-image` (its proportional tile-shrink mis-seats the centerline) and **never** a layout border on the pseudo (a 64px transparent border forces a 128px minimum box). Decor only: `pointer-events: none`, no layout, no animation; corner ink is paint-only overflow, so a mark-bearing host carries no `overflow: hidden`. The only ADDITIVE phase; owner preview | open     |
