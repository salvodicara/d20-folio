...
- `observedAt`: `2026-08-26T05:18:32Z`
...
Every resolved blob below comes from the exact prospective Task 4 tree. The test roadmap and
...
| `repositoryLeaseOwners[0]` | `docs/TEST_PORTFOLIO.md`                                              | `7f54b2f8f18667a04c308a910c3526d16a9decdc`                     |
...
`7b19401c6c1c4718a66566c585801c2c65d00640`) and the dependency baseline
...
- **Ownership:** public repository `d20-folio`; worktree
  ...
  `c476f2b3bf2a1cf9d504d8b1281d6979463f2f97`; pre-snapshot HEAD
  `4425731423bcd08b405046ff525b4aaffa58f235`; no private write. The charter owns
  ...
  in its plan.
- **State and receipt:** `executing`; the tree was clean at the pre-snapshot HEAD. Lease acquisition
  commit `0690ee5a31477397c752cc3110effd30b49af1c0` and Task 3 final head
  `4425731423bcd08b405046ff525b4aaffa58f235` have focused review/verification receipts. This Task 4
  commit is intentionally not self-embedded; Task 5 must advance the exact resulting candidate
  through independent review and verification before integration.
...
- **Acceptance:** Task 1 path/bootstrap tests; Task 2 deterministic state/replay tests; Task 3
  atomic runtime/CLI/rebuild tests; one routed status owner; exact authority blobs; pinned
...
- **Independent review:** Task 1-3 scoped reviews are accepted at their recorded commits. Task 5
  still owes specification/quality review of the complete fresh-base branch and every review fix;
  changed identity returns through review before verification.
...
- **Active writer lease:** `F0` → `program-supervisor-foundation`; exact exclusive paths are
  ...
  `docs/TEST_PORTFOLIO.md@7f54b2f8f18667a04c308a910c3526d16a9decdc`, reconciled through
...
- [ ] **Control plane:** a fresh supervisor reconstructs one valid program from the append-only
      ledger, exact authority manifest, Git/worktree/task inventory, and narrow lease cache; the
      detached program-control worktree and sole-writer heartbeat handoff are proven.
...
| Abandoned runtime staging directories, owner artifacts, temp files, or locks                                        | Preserve as recovery evidence until the owning PID is proven absent, the authoritative ledger/caches validate and rebuild, and an evidence-backed cleanup event names the exact artifact.        |
