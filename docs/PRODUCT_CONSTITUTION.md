...
### 1.1 Reconciled scope

[PRODUCT.md](../PRODUCT.md) owns the exact current scope: full BG3 behavioral fidelity for combat
and every transferable interaction in Folio's identity, D&D 2024, physical dice always allowed,
contextual freedom, the four permanent domains, multi-character membership, and complete
permission-scoped DM inspection distinct from the active actor. Those statements refine the
benchmark wording above. [DESIGN.md](../DESIGN.md) makes the complete creation and growth wizards,
linked receipts, reactions and undo, shared animated dice and integrated range/geometry obligatory;
generic lab forms never limit the target, and no animated 3D world follows from the dice
requirement.

**Theme.** Dark only, in EN and IT. Light paper is a printing treatment, never a second application
theme; the earlier two-theme obligation is superseded (owner, 2026-09-05).

Production remains usable until the complete V2 and its player-data migration are verified and the
owner authorizes switching. Neither documentation, a staging build nor an internal V2 cutover is
...
The application rolls dice (owner, 2026-09-03, reversing the v1.0 rule, which is now history).
Rolls are
...
(owner, 2026-06-10 → 2026-06-12; the concrete recipes live in `DESIGN.md`):
...
16. The register rule: premium visual registers (the hero altar, the carved cartouche, the
    ...
    (Owner-ratified across the wizard/cockpit campaign, 2026-06-11/12; the recipe ladder lives in
    `DESIGN.md`.)
17. No truncation: identity text is never mid-string ellipsized — swap to a shorter true form at a
    ...
    rule and canonical examples in `DESIGN.md`.)
...
> `CLAUDE.md` → "Architecture in one breath", `docs/ARCHITECTURE.md`, and
...
> All of the above shipped in the v1 production app and are being rebuilt on `v2` through the
> program in `docs/program/`. The command stands for every future module: the design system must
> admit it without re-architecture.
...
Amendment 2.4 (2026-09-09): reconciled with the one-program knowledge base — the dated prologues
are removed, the light-theme and two-theme obligations are deleted (dark only, owner 2026-09-05),
and the earlier no-roll, no-VTT and no-map remnants are gone.
...
| 2026-06-27 | 1.4     | Added §2.9 DM Tools Are An Optional Force-Multiplier: optional-never-mandatory + captivating; one convenient pane of live team stats; progressive disclosure (no info dump). (Its VTT and map clauses were reversed by 2.1.)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| 2026-09-06 | 2.3     | Reconciles the owner-approved P00 direction and P01 continuity clarification: device scope, dark-only EN/IT, BG3 behavioral fidelity and implementation depth, explicit prerequisite exceptions, and production retained until verified owner-authorized switch. Documentation of accepted decisions; no runtime or data migration attested.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 2026-09-09 | 2.4     | Reconciled with the one-program v2 knowledge base: the dated prologues are removed, the light-theme and two-theme obligations are deleted (dark only, owner 2026-09-05), the §2.2 dice reversal from amendment 2.1 is recorded as the standing rule rather than a dated note, and the authority model moves from four document roles to five (Constitution, Program, Map, History, Operations).                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
