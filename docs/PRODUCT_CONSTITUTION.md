...
- **The observability boundary is the automation boundary.** The table declares facts the app cannot
  observe — position, range, line of sight, cover, or which creature entered an area. Once those
  facts are supplied, every consequence determined by modeled rules and data is the engine's
  responsibility: propose the correct result, show it before commit, and keep a clear override and
  reversal for rulings and homebrew. Being overridable never excuses a missing deterministic default.
