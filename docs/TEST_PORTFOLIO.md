...
| Lane                 | Files                             | Notes                                                                     |
| -------------------- | --------------------------------- | ------------------------------------------------------------------------- |
| unit fast (`node`)   | 434 root `.test.ts` (54 guards)   | pure logic, codecs, guards; `tests/lanes.ts` lists the DOM-bound `.ts`    |
...
Count them with `find tests/unit -name '*.test.ts*' | wc -l`,
`grep -c "^\s*it(" tests/rules/*.test.ts` and `pnpm exec playwright test --list | tail -n 1`.
...
Still to die, with their representations: the 47 kernel test files (stage 6, with the mechanics
kernel and the old play surfaces), `automation-corpus`/`automation-compiler` and the coverage
