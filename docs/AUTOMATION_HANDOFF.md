...
## Ponytail convergence review (2026-08-20) — recorded debt, apply post-merge

The independent over-engineering review of the converged epic found ~1000 deletable lines.
Triage (orchestrator judgment, none applied post-gate to keep the validated tree frozen):

- SAFE DELETES (apply first): dead exports with zero callers anywhere — `tupleSchema` +
  the "tuple" kind + `conformExact` (exact-schema), `isMechanicsWorld`,
  `conformProgramRootReceipt`, `MECHANICS_PROGRAM_AUTHORING_SCHEMA_CONTEXT` +
  `mechanicsPhaseOrderedDependencies` + `conformMechanicsPredicate`;
  `worldStandingTargetMarks` (test-only, speculative).
- MECHANICAL DEDUP (one commit, full re-gate): `freezeDeep` copied 10x, `isRecord` 9x,
  `identifier` 7x, integer guards 6x, canonical-equality helpers 4x, three ~55-line
  plain-JSON walkers -> one shared `src/lib/json-guards.ts` home (~-310 lines).
- TEST-ONLY EXPORTS (move into test helpers): undoDamageReactionEntry, resetActionJournal,
  authorizeMechanicsRequester, locateMaterialResource, deriveTurnEconomyBudget,
  resolveExhaustion + applyLongRestExhaustion, deriveMechanicsRequirements,
  isMechanicsCompilerContinuationFor, topMechanicsPendingFrameMatches +
  advanceMechanicsPendingFrameSlot + finalizeMechanicsMaterialCleanup.
- JUDGED WRONG or DELIBERATE (do not apply): the respond/responses channel (the kernel's
  needs-response protocol consumes it for recorded observations; the UI consumer is the
  open follow-up, not dead code); replacing exact-schema with zod (the zero-dependency
  schema layer is a deliberate architecture choice, not accidental reinvention);
  `character-vitals` narrow world readers (they ARE the arbitration flip point);
  the 13 occurrence selectors (decide: adopt in production walks or delete — an
  adoption pass is the better outcome, not deletion);
  `slotMaxOverrides`/defaulted `characterWorldState` params (worth doing WITH the
  dedup commit).
