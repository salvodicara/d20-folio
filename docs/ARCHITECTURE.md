...
- **Auto-narrated capture + universal resolution (the sheet → combat-state seam).** Committing any
  ...
  **Target-bound standing effects use one append-only ledger.** `EncounterState.effectOps` stores only
  ...
  together. Target grant projection exposes `roll-die-adjustment`, Speed deltas and `healing-blocked` to
  the resolver; campaign and solo HP reducers consume the same healing flag, so offline PCs and monsters
