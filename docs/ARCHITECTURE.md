...
- **`@pack/item-art` — private item-illustration URLs behind a corpus gate**
  (`packItemArtAliasTarget()`; pack mode → `content-pack/data/item-art.ts`, SRD-only →
  `pack-empty.ts`). The public aggregate `src/data/item-art.ts` keys every plate by
  **typed corpus id** (`equipment:<id>` / `magic:<id>`) so identically named mundane and magic
  ...
  Compendium resolves art only for an opened reading leaf: dense lists and Add Item modals remain
  icon-led and make zero image requests. The aggregate derives its expected keys from every active
  composed equipment + magic-item row and enables plates only when the set is complete; a partial
  public/private corpus therefore renders the common no-art anatomy everywhere instead of making
  equivalent leaves unpredictable. Plates are 672×840 WebP at ≤45,000
...
`var(--cond-<id>-ink)`), VALUES (dice / save DC / measured distance-duration → `.rt-value`),
Advantage/Disadvantage (→ `.rt-adv`/`.rt-dis`), and the Action / Bonus Action / Reaction triad
(→ `.rt-action`/`.rt-bonus`/`.rt-reaction`) into keyed React nodes. It sits at the **rules-text
...
- **TOTAL codec — no silent drops (design §5.5).** Every COLLECTION in the envelope is total: the
  parse never skips an element. Each key outside the closed worlds (`KNOWN_BUILD_KEYS`,
  `KNOWN_STATE_KEYS`, the per-entry key lists) is PRESERVED verbatim in an `unknown` bucket on the
  character / session / entry / class entry and written back last, so an older client can never trim
  a document a newer one wrote. A structurally malformed element QUARANTINES the whole document —
  `parseCharacterEnvelope` returns a typed `failure` (code plus path) instead of skipping it, since a
  skip would write a SHORTER array back over live data. The quarantine path is:
  `parseStoredCharacter` throws a `TypeError` whose message is the codec's `code:path` →
  `subscribeToCharacter` → the subscription's `quarantine(message)` → the diagnostics report
  (`diagnosticsLog`), which stops persistence for that document instead of repairing it. Totality is
  STRUCTURAL; the remaining, enumerated value-level normalizations are listed in
  `docs/CHARACTER_SCHEMA.md` → "The codec (implementation contract)", which also owns the failure
  taxonomy.
...
- **Security (`v2`, stage 4) — `firestore.rules` is identity, membership, ownership and shape;
  ...
  | `users/{uid}/characters/{id}`              | read/write under the revision CAS, empty parent state, exact projection | read when the roster names this character                                                                                                                                                                                                    | same as a member                   | owner-level on an UNSHARED character; a published character's build change and its deletion still require the owner, because `public/sheet` is owner-only and both invariants are atomic | —          |
  ...
  | `campaigns/{id}/encounters/{eid}`          | —                                                                       | read; append to `log` — the stored log must stay a PREFIX of the written one, so existing entries are byte-frozen and only the tail grows; an EMPTY stored log is exempt (a rules slice `log[0:0]` errors instead of yielding an empty list) | create, rewrite/checkpoint, delete | DM-level                                                                                                                                                                                 | —          |
  ...
  the table; nobody, member or DM, can touch another user's documents at all. An empty stored log is
  trivially a prefix of anything and has nothing to protect, which is why the fence's empty-log
  exemption costs nothing.

  WRITERS: `src/lib/combat-io.ts` (refs, create, append via `arrayUnion`, subscribe, the checkpoint
...
the SAME 14 eager chunk families: no lazy boundary leaked and no asset corpus entered the startup
path. The growth is legitimate product weight from filling the formerly-empty equipment prose in
the statically resident EN facts catalogue, the shared spell/item tag seam, the expanded rules-prose
reading grammar, and 0.11 KB gz of CSS. The 794 KB ratchet leaves about 2.5 KB deterministic
headroom; the item WebPs remain outside the eager closure and the all-or-none art gate stays dormant.

The 2026-08-12 whole-product UI/UX closure preserves the existing **65 KB entry / 794 KB eager**
ceilings. The integration gate first measured an invalid 87.1 KB entry because one eager shell glyph
made the complete 52 KiB fantasy-icon authoring module statically reachable. The closure split the
four persistent shell glyphs into `folio-shell-icons.tsx` and changed the command palette from an
always-mounted static import to a post-mount-warmed dynamic import; the entry and eager closure then
returned below their existing ratchets without changing the approved iconography. The completed
...
The 2026-09-02 combat P1 data-safety round raised the eager closure 844 → 850 KB and the precache
9501 → 9525 KiB. A fresh `origin/main` build measured **843.1 KB gz / 17 families** and **9497 KiB /
334 entries** — already exact-fit against 844 (the 840.89 figure above was an under-recording), so
any eager addition flipped the gate. P1 adds the eager diagnostics layer (`src/lib/diagnostics`,
`diagnostics-io` reporter, ADR-0008) and the total character codec (typed failures, unknown
buckets, `codec-failure.ts`): the composed build measures **844.7 KB gz / 16 families** eager and
**9511 KiB / 334 entries** precache (+1.6 KB gz, +14 KiB). Both ceilings keep ~5 KB / ~14 KiB
never-exact-fit headroom.
