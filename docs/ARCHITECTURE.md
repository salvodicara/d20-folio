...
- **Security (`v2`, stage 4) — `firestore.rules` is identity, membership, ownership and shape;
  ...
  | `campaigns/{id}/encounters/{eid}`          | —                                                                       | read; append to `log` — the stored log must stay a PREFIX of the written one, so existing entries are byte-frozen and only the tail grows | create, rewrite/checkpoint, delete | DM-level                                                                                                                                                                                 | —          |
