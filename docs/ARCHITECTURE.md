...
8392 KiB. The growth is genuine feature weight, not a structural leak: the eager closure holds the
SAME 14 chunk families it did at 787.03 (no lazy chunk became statically reachable), and the +1.67 KB
is the 28 new `breakdown.why.*` chrome strings (the `common` bundle is eager by design) plus the
shared `WhyProse` component and the tip's accordion. Measured: eager 788.70 KB gz (JS 711.7 + CSS
77.0), precache 8390.11 KiB / 321 entries.
