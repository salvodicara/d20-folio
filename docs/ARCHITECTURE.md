...
- **Auto-narrated capture + universal resolution (the sheet → combat-state seam).** Committing any
  ...
  **Any creature** switch supports friendly fire, unusual rulings and homebrew. A weapon swing stays one
  ...
  `combat/state`, reduces typed HP/temp/condition effects, and merges only those combat fields. The target
  ...
  **Target-bound standing effects use one append-only ledger.** `EncounterState.effectOps` stores only
  ...
  index), catalogue source ids, immutable cast bindings and duration. `foldCombatEffectOps` is the one
  active-state projection; every reader passes it through `resolveCombatEffectGrantSources`, which projects
  ...
  transaction applies/revokes operations and any max/current-HP delta. Damage runs the generic persistent
  reducer for universal resistance, exact post-resistance transfer, one-shot zero-HP floors and
  successful-hit reactions. A hit is carried independently from its damage amount, so a modeled
...
- **`DEV_BYPASS_AUTH` parity replica** — the cheaper screenshot/E2E lane; fixtures/scenarios are INITIAL SEEDS, not a parallel runtime.
  ...
  omits `COMBAT_SESSION_KEYS`, while `combat/state` owns HP/temp/conditions/death saves/initiative/round,
