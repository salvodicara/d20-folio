...
**`@pack`** (plus exactly ONE sub-entry, `@pack/monsters`) is the whole seam:
...
- **`@pack/monsters` — the ONE lazy sub-entry** (`packMonstersAliasTarget()`; pack mode →
  `content-pack/data/monsters.ts`, SRD-only → the SAME `src/data/pack-empty.ts` stub, which
  exports `packMonsters: []` for both aliases so there is no second empty file to keep in step).
