...
This is the only delivery plan for d20 Folio v2, written in the present tense: every row says what
a block must deliver, never what already happened. Execution state — current block, closed SHAs,
open gates — belongs to [`docs/PROGRAM_STATUS.md`](../PROGRAM_STATUS.md) and to the short handoff
[`NEXT.md`](NEXT.md). Owner decisions live in [`DECISIONS.md`](DECISIONS.md); the closing controls
of a block are in [`CHECKLIST.md`](CHECKLIST.md).

`Pxx` names a delivery block, not a session: required corrections and open gates continue the same
block, never creating a new `Pxx` and never licensing the next one. Block P00, closure of the
external HTML laboratory, is finished — it produced the approved mock
`d20-folio-html-0.9.3-2026-09-06` and its manifest, described in
...
- **V2 is a completely new application.** Existing code, a reachable old route or an old test has
  no authority over it. Build, replace or reuse only to satisfy the new domain and the approved
  experience: one authority per fact, explicit responsibilities, verifiable transitions, no
  duplicated state and no alternative old path.
- **No legacy combat bridge.** Production keeps running separately; its data is only the input to a
  recoverable migration (snapshot → dry-run → idempotent apply → verify). A proposal that exists
  only to make the old product work inside V2 is out of mandate: delete it from the plan.
- **References.** Reproduce every transferable Baldur's Gate 3 behaviour — organisation, sequences,
  feedback, interaction quality — in Folio's identity, and adopt the useful capabilities of D&D
  Beyond, Roll20 and Owlbear. D&D 2024 governs the rules; BG3 rule variants are never imported. The
  mock guides composition; it is not a ceiling on depth.
- **Theme and language.** Dark only, IT and EN. Light paper is for printing, not a second theme.
- **Owner gates.** Integration into `v2` never authorises deployment, production writes, real-data
...
A block starts from the current owner message, the identified approved mock (tag, manifest and hash
verified now, never a pre-filled historical baseline), the pertinent screenshots and review, and the
current execution state and runbooks. The block owner owns only its assigned modules and the
documents owning the facts it changes; it integrates other boundaries without reverting them and
preserves other checkouts and edits. Before code: a small plan and useful behavioural tests. After
code: the block's proofs, the pertinent repository gates, review, and curated before/after
screenshots delivered in chat — 1440×900 and 1280×800, EN/IT dark where the case applies, phone
where consultation and updates matter, with no invented mobile HUD parity. Verify full Italian
names, crop and fallback, keyboard and focus, and undo, error, offline and permission paths where
they matter. Anti-drift compares layout and component against the identified mock screenshots, not
a general resemblance. Every visual block also passes the simplicity check — main task, essential
data, primary action, secondary detail — without duplicating controls or creating competing paths,
and disclosure keeps every case reachable without hiding warnings, costs or undo. The block ends
with: commit and destination when the runbooks authorise them, tests run and skipped, screenshots
approved or pending, coverage moved from "HTML" to "runtime proven" only with evidence, execution
state in its single owner document, product facts in the documents that own them, and a rewritten
[`NEXT.md`](NEXT.md). No implicit deployment; never copy a historical verdict forward.
...
| P23 Connections and reminders     | P22 + cost choice | calendar IO adapters, settings                     | Google opens a pre-filled event to save and ICS covers the rest, with single, series and all scopes; reminders a day and an hour before, with consent and idempotence.               |
...
1. Read the single current execution state, with its assigned block **and bounded outcome** and its
   exit evidence. If the outcome is not finished — tests, review, required screenshots, dependencies
   or gates missing or failed — assign **the same block and the same outcome** plus the recovery
   work. Never pick other work from the graph to step around an open gate.
2. If the outcome is finished but the block still has open outcomes, take the first one in the
   declared order. P14a, P14b and P14c each walk the eight rows of the E → outcomes table below from
   top to bottom; starting P14b before the whole of P14a, or P14c before the whole of P14b, stays
   forbidden. Any further outcome needed is registered in order before it is executed, without
   altering the dependencies.
3. Only when the whole block is finished, walk the block table from the top and take **the first
   unfinished row whose dependencies are all finished and proven**. The order is P01, P02 …
   P08a → b → c, P09, P10, PD, P11a → b, P12 … P14a → b → c, P15 … P17a → b, P18 … P27a → b → c,
   P28 → P29 → P30. A dependency range requires every row and sub-block inside it. Even when
   several rows are eligible, exactly one is assigned: the first.
4. If the state says the block is finished but no row is eligible while work remains, report the
   inconsistency or the missing dependency and keep verifying the current block. Never invent a
   completion or reorder for convenience. After P30 and all gates, deliver the final record; do not
   invent a P31 and do not deploy.

Every handoff shows the selection and the evidence for its dependencies and repeats this same
recursive rule. One row or outcome may need several sessions, always resuming the same outcome until
it is satisfied; a later change reopens the affected proofs and gates.

## E01–E22 assigned to P14a/b/c

<!-- source: full-lab/docs/RULES-EXCEPTION-CATALOG.md; assignment table in AGENT-PROGRAM.md -->

This table decomposes the work inside P14a/b/c; each cell is a bounded outcome with its own inputs,
proofs and handoff, and P05–P08 own their families' editors, never the execution handlers. For each
outcome record an ordinary case, a boundary case and a composition with another row, plus the
contextual manual counterpart on the same facts, costs, receipt and undo.
...
Capabilities already assigned to P01–P30 stay mandatory. A required interaction that is absent is a
gap, never something to delete in order to reach a complete count.
...
The mock shows some generic paths; depth is realised in the implementation blocks and its
simplifications never become the product's limit. Judge flow completeness, readability and pleasure
of use alongside correctness; a technical choice never justifies a silent product cut.

- **P10 creation and P11a growth:** complete wizards with real choices — origin, class,
  proficiencies, ability scores, equipment and pertinent magic for creation; subclass, multiclass,
  feats, features and magic derived from the character and level for growth. Explained
  prerequisites, visible consequences, before/after comparison, returning to a step without losing
  the draft, cancel and resume, summary and confirmation. Free text exists only for voluntary
  exceptions; a generic form is not an acceptable guided path.
- **P14a–c resolution:** action, targets, roll, costs, consequences, reactions and causal correction
  stay bound to the same facts and the same receipt, under D&D 2024 rules.
- **P15 dice and attack:** animated 3D digital dice with readable result, faces and modifiers and a
  shared animation, plus the physical or manual alternative and a reduced-motion variant. The
  animation presents an authoritative result without regenerating it. Action → target → range and
  conditions → roll → consequences and reactions → undo is one continuous experience; normal, long
  and out-of-range distances and pertinent advantage or disadvantage are explained.
- **P17a–P18 integrated geometry:** P15 prepares the contract, presenter and the path with no map;
  P18 proves it on the shared map with distance, range indicators, targets and areas at the
  authoritative scale. Range, movement and area stay distinct; the integrated capability is not
  closed by the P15 proof alone, and no P15 → P18 → P15 cycle is created.
- **P28/P29 acceptance:** prove creation → growth → encounter → attack at normal, long and
  out-of-range distance → digital or physical dice → consequences and reactions → undo, with
  runtime evidence of the real branches.
...
P05, two in P06, three in P07 and two in P08, and every family gets the same coverage — autosave,
sharing, revocation, copies, versions, import/export, print — not merely "it has an editor". P23
must not become a paid infrastructure project; calendar integrations and the ACLs of new copies
need targeted research when implemented.
