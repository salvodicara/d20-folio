...
<!-- source: chat 2026-09-09 evening (owner) -->

«Utilizzerò GPT per generare tutte le immagini raster, che devono essere molto simili, ispirate a
quelle di Baldur's Gate 3, però ovviamente non uguali, così non abbiamo problemi legali.» ·
«l'app non deve mai reinventare la ruota... gli strumenti a cui ci ispiriamo — Baldur's Gate 3,
D&D Beyond, Owlbear, Foundry, ecc. — già lo fanno bene. Dobbiamo semplicemente copiare e riadattare
tutti i pattern.»

- **Raster art.** Raster images for spells, actions and content are generated by the owner's GPT
  workflow, in a style strongly inspired by Baldur's Gate 3, never a copy of any asset. The legal
  boundary: patterns and taxonomy are copied, assets are not. This answers the licensing question
  §9.4 of the PD research.
- **Rule 30 is enforced structurally.** The owner has said it «mille volte» and it was still skipped,
  so the fix is a gate, not louder text: the reference dossier is a mandatory section of every block
  spec, checked by `tests/unit/docs-budget.test.ts`, and a required row in
  [`CHECKLIST.md`](CHECKLIST.md) §A.
...
- **The experience is too cold.** P10 feels "troppo fredda, distaccata e amministrativa", like a
  ...
  The mock was chosen for the battle map, the bridge and those colours; approving it did
...
- **Progressive disclosure, both sides.** The expert acts fast with no mandatory explanation slowing
  ...
  playing. Neither costs depth. BG3 and D&D Beyond are studied as **primary and binding**
  references.
...
- **Short handoffs.** End-of-session prompts were long because contracts and history were copied
  inline; that is dropped for a short handoff kept in documents.
