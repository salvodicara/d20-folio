...
`initializeRuntime` builds the complete runtime in a unique sibling staging directory, creates `state/`, `ledger/`, `handoffs/`, `evidence/`, and `recovery/` with mode `0o700`, and writes/fsyncs all `0o600` files and directories before publishing anything. After the pre-claim crash hook it claims the previously absent root with one atomic no-replace `mkdir(root, 0o700)`; `EEXIST` fails without mutating the existing target. The winning initializer fsyncs the empty claim, publishes only its already-durable staged child directories into that claimed root, fsyncs the completed layout, and removes its now-empty staging directory. It canonicalizes the fully validated bootstrap input, records its SHA-256 fingerprint in the snapshot, and preserves the canonical bytes at `evidence/bootstrap-input-<fingerprint>.json`; this immutable identity covers every authority, charter, state, receipt, worktree/repository pointer, and lease field rather than a selected subset. Two initializers cannot both win. A crash before the root claim leaves only non-authoritative staging and permits a retry; a crash after the claim preserves both the incomplete root and remaining staging for manual quiescent recovery. Consumers reject that incomplete layout, and initialization never deletes or adopts an ambiguous pre-existing root. Abandoned staging directories are surfaced for later evidence-backed cleanup only after their PID is absent.

Every later public read or write acquires `root/.write-lock` with exclusive-create semantics. It first writes and fsyncs one complete unique `.write-lock.owner-<pid>-<uuid>` file, fsyncs the root, then atomically hard-links that inode to the canonical lock path. A crash before publication leaves no canonical lock; the complete owner artifact is surfaced as `recoveryState.abandonedLockOwners` only after its filename/record PID agrees and that PID is provably absent, and it is never deleted automatically. Live or `EPERM` owner artifacts are skipped. Every directory entry in the reserved `.write-lock.owner-` namespace must satisfy the complete canonical filename, PID, UUID, regular-file, mode, and record-ownership contract; malformed or unsafe entries and indeterminate PID probes fail closed, while unrelated root entries remain outside that namespace. The canonical lock contains schema version, PID, and acquisition time; acquisition retries for a bounded interval, reports a live owner precisely, and may recover a lock older than 30 minutes only when that PID is provably absent. Recovery first attempts one deterministic hard link from the exact canonical lock inode to `recovery/write-lock-stale-<sha256>.json`; creating that exact-inode evidence is the atomic recovery claim. `preserveExactLink` distinguishes the new claimant from an observer of pre-existing exact evidence. Only the new claimant may re-lstat the canonical inode, unlink it when device/inode still match, fsync the root, and retry acquisition. An observer never unlinks and returns busy; if the claimant crashes after its durable claim, later attempts fail closed for manual quiescent recovery. A changed or missing canonical inode after claiming is preserved and fails closed, so a successor lock is never removed. Always release a live lock in `finally` without unlinking a replacement inode.
...
test -z "$(find /Users/salvatoredicara/Workspace/Codex/d20-folio-program \( -name '*.tmp' -o -name '.write-lock' -o -name '.write-lock.owner-*' \) -print)"
find "$runtime_check_dir" -depth -delete
```

Expected: validation prints `"valid":true`; both rebuilt caches are byte-identical; the authoritative ledger hash is unchanged; root/directory and file modes remain `0700`/`0600`; and this normal-operation proof has no lock, owner, or temp residue. On crash recovery, do not use the zero-residue assertion as cleanup authority: require every atomic-replace residue to appear in `recoveryState.abandonedTemps` and every complete dead-PID owner artifact to appear in `recoveryState.abandonedLockOwners` only after exact target/type/mode/content/filename/ownership validation, preserve it for evidence-backed cleanup, and fail closed on unsafe, mismatched, live/indeterminate, malformed reserved-prefix, or unclassified candidates.
...
Routine background notifications remain failed-run-only. When a later wake reaches a genuine owner gate, the supervisor first appends the exact cycle's `owner-gate-recorded(pending)` while the task is in `verification`, then transitions that same identity to `owner-gate`, pauses this exact automation before the owner answers, exposes the dedicated task as needing attention, and ends with the concise product/visual decision plus recommendation and images when visual; it never leaves a gate only in the ledger. Resumption after the owner answers appends exactly one terminal `approved` or `rejected` record while still in `owner-gate`, follows only the corresponding integration or evidence-backed blocker transition, and then restores the routine failed-run-only policy when continuation remains valid.
