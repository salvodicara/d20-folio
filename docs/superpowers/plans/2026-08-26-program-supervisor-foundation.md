...
- full replay uses exactly one `rev-list --reverse --parents` plus three buffered `cat-file --batch` processes for commits, unique trees, and unique event/bootstrap blobs, so Git subprocess count is constant as the chain grows; the invariant bootstrap blob is fetched once and validated against every tree OID;
