...
Claude wrote it as the research half of block PD, under the owner's split of 2026-09-09: Astra owns every visual and taste decision, Claude owns the rest (`docs/program/DECISIONS.md`; `docs/program/PROGRAM.md` § "Who does what in PD").
It states evidence, observable patterns and testable rules, and contains no visual verdict — no colour, layout, typography or art style; §9 and §10 name what stays open.
The evidence sits in [`2026-09-09-pd/`](2026-09-09-pd/) as **E1** `bg3-character-creation.md`, **E2** `bg3-combat-ui.md`, **E3** `dnd-beyond-builder-and-sheet.md`, **E4** `progressive-disclosure-and-onboarding.md` and **E5** `current-creation-experience.md`, copied verbatim and reformatted by prettier.
Citations read `(E4 §1; S3)` — evidence file and section, then the source in § Sources, which carries the URL for every claim with a number, a quotation or a defect.
Astra's next deliverable, per PROGRAM.md's PD block: the redesigned creation screens and the raster art direction with concrete examples and coherence criteria; the owner's verdict closes PD.
...
- **Already serving the brief** (E5 §7): bidirectional navigation with managed focus, non-destructive branching, an attributed rule exception, read-before-choose where a catalogue carries text, and EN/IT parity. Nine areas violate an owner principle: entry, identity, imagery, consequence preview, copy register, error model, expert lane, beginner lane, draft safety.
...
| 10  | Pre-commit forecast of what a choice changes    | Fire Emblem (S15); ItB   | Understanding consequences   | A preview diff reports AC, speed, skills, slots    |
...
3. Maths shown but not applied — the Short Rest panel states what each Hit Die returns, then demands manual HP entry (E3 §2).
...
**8.1 Recommended-build data — exists, wired to the wrong flow.** `src/data/quickbuild.ts` declares one ready-made level-1 build per class (`abilityOrder`, `boost`, `classSkills`, `cantrips`, `spells`, `choices`, `languages`, `lineage`), guarded so every composed class has one and extended by the pack, and `src/lib/quickbuild.ts` is the pure applicator — both imported only by the legacy wizard (E5 §1). **Gap:** the applicator targets the legacy `creationChoiceSlots` seam, not P10's `OriginChoice` seam, so a new module beside `src/lib/character-creation/` must replay a preset through `selectCreationSource` and `answerCreationChoice`, pinned by a test that every preset is `valid`.
...
- **S13, S14, S26** PC Gamer, https://www.pcgamer.com/`<slug>`— S13`baldurs-gate-3-abilities-proficiency-and-skills/`; S14 `baldurs-gate-3-respec-guide/`, with Escapist and GameSkinny on changing race (S17); S26 `games/baldurs-gate/baldur-s-gate-3-s-spell-tooltips-have-been-lying-to-you-…` (partially verified)
- **S15** Fire Emblem Wiki, combat forecast — https://fireemblemwiki.org/wiki/Combat_forecast · **S16** Into the Breach enemy intentions — https://atomicbobomb.home.blog/2020/05/17/into-the-breach-enemy-intentions/ · **S17b** Slay the Spire Wiki — https://slaythespire.wiki.gg/wiki/Module:CardTooltip
- **S18** Game Developer, an inclusive character creator — https://www.gamedeveloper.com/art/building-an-inclusive-character-creator-for-the-fantasy-world-of-baldur-s-gate-3 · **S19** TheGamer, 88 years in the creator — https://www.thegamer.com/baldurs-gate-3-player-time-spent-in-character-creator-88-years-stats/
