...
- [x] **Bounded spell-pool execution closure — SHIPPED 2026-08-04.** The 2026-07-06 bridge had only opened the picker and debited its tracker; it did not actually rejoin spell resolution. The picked spell now materializes as the ordinary resolved spell action before target review and flows through the one cast transaction: action economy, live affordability, CombatResolver, deterministic damage/healing/conditions, concentration, structured spell-name log/Chronicle provenance and exact undo. `castPoolSourceId` disambiguates separate homebrew lists sharing one tracker; item-only `always-prepared-spell` visibility cannot spend unrelated class slots; redo bails rather than overdrawing a resource spent elsewhere. Typed source overrides model the items' fixed save DCs and War God's Blessing's concentration-free 10-round persistence, including Spiritual Weapon's recurring action and timer expiry. The regression covers solo exact payment/undo/log, encounter resolve-before-spend, stale-redo refusal and persistent source-cast expiry.
...
- [x] **REMAINING DICE-ENTRY GAPS — CLOSED 2026-08-04:** Wholeness of Body now declares its Martial Arts die + WIS heal and self target; Form of Dread's Facsimile of Life declares `1d10 + Warlock level` Temporary HP, self-target Frightened removal, its 10-round lifetime and early Incapacitated expiry. Both reuse the universal roll-entry resolver: the app never rolls, then applies the reviewed result with undo and max-wins Temporary-HP semantics. Healing Hands and Heightened Patient Defense use the same path. No feature-specific runtime branch was added.
...
- [x] **PS-G (rail double-listing) — the Advantages rail listed one advantage twice, in two
      ...
      restating the label (`Adv. Advantage on Initiative rolls.` /
      ...
      new authoring guard in `advantage-vs-slug.guard.test.ts` (no grant description may open with
      Advantage/Disadvantage/Vantaggio/Svantaggio, both roots, both locales). The 41st —
      `robe-of-eyes.grants.2`, both locales — hid its polarity behind an `All-Around Vision: ` label
      prefix, which the guard's first `^`-anchored matcher structurally could not see; the matcher
      now skips an optional `<Label>: ` prefix before the anchor, so a label can never license the
      polarity behind it. Re-scanned all 208 `*.grants.*` descriptions (both roots × both locales)
